...
test('10タップで5回短縮し、拘束開始から500msで脱出する', t => {
 ...
 function tap(id){for(const type of ['pointerdown','pointerup']){const e=new Event(type);Object.assign(e,{pointerId:id,isPrimary:true,button:0,clientX:id%2 ? 20 : 240,clientY:id%2 ? 20 : 240,pointerType:'touch'});(type==='pointerdown'?board:window).dispatchEvent(e);}}
 for(let i=0;i<10;i++){if(i)advance(i%2 ? 60 : 20);tap(i+1);}
 assert.equal(play.trap.target,.5,'5組すべてが短縮として認識される');
 advance(119);assert.ok(play.trap,'499msではまだ拘束中');
