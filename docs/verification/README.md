...
固定面チュートリアル。120テスト成功。browser-tutorialで320/390/576幅、説明停止・自動終了・タップ・非表示復帰・記録非保存・基準再計測・拒否/無信号を確認。browser-tutorial-offlineで画像・起動・接触説明、既存ガイド3幅・記録両難易度も成功。レビュー2件（説明キュー・SE二重発音）修正済み。正式URL公開確認待ち。
