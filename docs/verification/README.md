...
116テスト成功。実物理で初速3・無入力の1.02マス反発と、石への衝突ダメージを確認。browser-candy-fullで満タンの通知・温存・再接触案内、browser-guideで3幅の説明表示・床等高を確認。レビュー指摘なし。実装6bfe8d7をmainへpush済み。2026-09-13 18:25 JST時点でGitHub Pagesチェックはqueued、正式URLはv0.4.8。公開後は変更資材の一致とbrowser-candy-full.mjsをBASE_URLで確認する。
