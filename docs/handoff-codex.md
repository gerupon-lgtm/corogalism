...
**フェーズ2のローカル実装・自動検証が完了。次はデプロイとT-209の実機試遊です。**
ローカルのバージョンは `v0.2.0`。フェーズ0は参照用のまま維持しています。
...
| T-209 | 未実施。Pixel 6a / iPhone XRで発案者が試遊する |
| フェーズ2の公開 | **未実施**。今回の作業フォルダには `.git` がなく、コミット・pushをしていない |
...
- 外周は標準壁。配置比率とHP・物理・難易度の数値は変えていない。
...
1. Git管理された公開用チェックアウトに今回の変更を反映し、差分を確認する。
2. コミットメッセージに `T-205,T-206,T-207,T-211,T-213,T-214,T-217` を含めて公開する。
3. HTTPSでPixel 6aとiPhone XRから起動。許可・拒否・キャリブレーションを確認する。
4. 両端末で素材の見分けやすさ、被弾表示、残り時間、停止・復帰、2回までのコンティニューを試す。
5. 「単調さが解消したか」「無理ゲーになっていないか」を判断し、T-209へ記録する。
