...
- T-255 / v0.5.8: 最初の開始操作で全画面＋縦向き固定を要求。対応しなくても開始可能。解除後は自動再要求しない。

- T-254 / v0.5.7: 本編・検証ページの縦向き固定をベストエフォートで追加。docs/portrait-lock.md参照。壁材配置ルールは変更なし。

- T-253: おためし7に広いカーブ＋重力・反重力の補助面を追加。旧タイムトライアルは残す。実機試遊待ち。

- T-252: おためし6に通常壁・氷＋砂タイムトライアル。直前だけ砂、時間計測・設定別自己ベスト。本編の壁ダメージは未変更。
...
- 公開版は **v0.5.7**。正式URL: https://corogalism.sikumilab.com/ 。公開アプリbdb7e7d、Pages35715972863成功。公開証跡は[deployment.md](deployment.md)。
