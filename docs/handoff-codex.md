...
- T-257 / v0.5.10: 実機評価で余白を縮小。全画面時は既存余白込みの合計32px（safe-areaが大きければ優先）。T-256の追加32pxを置き換える。

- T-256 / v0.5.9: 全画面時にカメラ回避用の上余白を最低32px確保。端末safe-areaが大きければ優先。小画面の盤面も余白分縮小。
...
- 公開版は **v0.5.10**。正式URL: https://corogalism.sikumilab.com/ 。公開アプリe1fa81d、Pages35719269382成功。公開証跡は[deployment.md](deployment.md)。
- 直近の自動検証は135テスト成功。開始時の全画面15経路・縦向き固定8経路を正式URLで模擬確認済み。ローカルのチュートリアル320/390/576幅・センサー3経路も成功。実端末での全画面・固定の成功可否は端末とブラウザに依存する。
