...
try{
 ...
 if(sensor)for(let i=0;i<14;i++){await page.evaluate(()=>emit());await page.clock.runFor(140);}
 ...
 for(let i=0;i<10;i++) { if(i)await page.clock.runFor(i%2 ? 60 : 20); await page.touchscreen.tap(board.x+board.width*(i%2 ? .75 : .25),board.y+board.height*(i%2 ? .75 : .25)); }
 ...
 console.log(`PASS native touch: ${sensor?'tilt':'pointer'}, 10 taps produce 5 reductions, escape at 0.5s (frame tolerance).`);
 await ctx.close();
 }
