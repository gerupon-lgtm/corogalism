...
try{
 ...
 await page.goto(base);assert.equal(await page.locator('.badge').innerText(),'v0.4.3');
 await page.evaluate(()=>navigator.serviceWorker.ready);await page.waitForFunction(()=>navigator.serviceWorker.controller);
 const cache=await page.evaluate(async()=>{const names=await caches.keys();return {names,files:(await (await caches.open(names[0])).keys()).length};});assert.equal(cache.files,73);
 ...
 assert.deepEqual(errors,[]);console.log('PASS: formal URL v0.4.3, all 73 resources saved, offline navigation/gameplay/BGM available; no page errors.');
